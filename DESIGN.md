...
### Dispatch Index (home writing)
Minimal typographic block for Substack posts — proof via text after image-heavy cases. Lives inside a soft glass card (`--radius-md`, hairline linen edge, `backdrop-filter: blur(20px)`, peach/pine radial mesh wash, inset top light-catch) — same material family as the cases gate, without the gate lift shadow. The peach Merriweather section title ("first layer club") is followed by the newsletter thesis as a quiet muted Heebo paragraph. Each dispatch is a single column within `--measure`: uppercase micro-label date on top, large Merriweather title (same scale as the cases headline) as the protagonist. No thumbnails, no issue rail, no summaries, no buttons — the only CTA is a quiet peach "subscribe" text link with an arrow. Scroll-reveal matches cases (`data-reveal`, stagger 40ms). Hover: title → peach; subscribe underlines on hover. EN surfaces a one-line note that posts are in Portuguese. `prefers-reduced-transparency` drops blur to a solid ink mix.
